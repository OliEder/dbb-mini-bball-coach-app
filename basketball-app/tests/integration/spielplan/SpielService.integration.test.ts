 ...
 *
 * v7.0: getSpieleByTeam() nutzt TeamLigaParticipation → liga_id
 */
...
describe('SpielService Integration Tests', () => {
  ...
  // v7.0: Liga-ID für TeamLigaParticipation
  const testLigaId = 'liga-integration-test';
  ...
  describe('Contract: getSpieleByTeam → Database', () => {
    it('sollte alle Spiele eines Teams chronologisch sortiert zurückgeben', async () => {
      // Given: Spiele in verschiedenen Daten (v7.0: mit liga_id!)
    ...
    it('sollte Filter korrekt an Database weitergeben', async () => {
      // Given: Heim- und Auswärtsspiele (v7.0: mit liga_id!)
  ...
  describe('Contract: Team-Statistiken', () => {
    it('sollte Statistiken korrekt berechnen', async () => {
      // Given: Verschiedene Spiele (v7.0: mit liga_id!)
      await spielService.createSpiel({
        liga_id: testLigaId,
        heim_team_id: testTeam.team_id,
        gast_team_id: gegnerTeam.team_id,
  ...
  describe('Contract: Concurrent Operations', () => {
    it('sollte gleichzeitige Operationen korrekt verarbeiten', async () => {
      // Given: Mehrere gleichzeitige Create-Operationen (v7.0: mit liga_id!)
      const now = Date.now();
      const promises = Array.from({ length: 10 }, (_, i) =>
        spielService.createSpiel({
          ...
          heim_team_id: testTeam.team_id,
          gast_team_id: gegnerTeam.team_id,
          datum: new Date(now + (i + 1) * 24 * 60 * 60 * 1000),
          heim: testTeam.name,
          gast: `Gegner ${i}`,
          ist_heimspiel: i % 2 === 0,
          status: 'geplant',
  ...
  describe('Contract: Data Integrity', () => {
    it('sollte Datenintegrität bei komplexen Workflows wahren', async () => {
      // Given: Create → Update → Filter → Stats Workflow (v7.0: mit liga_id!)
