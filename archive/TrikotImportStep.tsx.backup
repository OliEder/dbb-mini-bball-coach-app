/**
 * Trikot Import Step - KORRIGIERT
 * 
 * CSV-Import für Trikots
 * Zurück-Button geht zu spieler
 */
...
export function TrikotImportStep() {
  const { trikot_csv, setTrikotCSV, setStep, selected_team_name } = useOnboardingStore();
  
  const [file, setFile] = useState<File | null>(trikot_csv || null);
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      setFile(selected);
      setTrikotCSV(selected);
      setResult(null);
    }
  };

  const handleDownloadTemplate = () => {
    const csv = csvImportService.generateTrikotTemplate();
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'trikot_vorlage.csv';
    a.click();
  ...
  const handleImportPreview = async () => {
    ...
    setImporting(true);
    try {
      const importResult = await csvImportService.importTrikots(file, tempTeamId);
      setResult({
  ...
  return (
    <div>
      ...
      <p className="text-gray-600 mb-2">
        Importiere deine Trikot-Liste als CSV-Datei.
      </p>
      ...
      <div className="mb-6">
        <label htmlFor="trikot-csv" className="label label-required">
          CSV-Datei auswählen
        </label>
        <input
          id="trikot-csv"
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className={`font-semibold mb-2 ${
                ...
                {result.success 
                  ? `✅ ${result.count} Trikots validiert!`
              ...
              {result.success && (
                <p className="text-sm text-success-800 mb-2">
                  Die Trikots werden beim Abschluss des Onboardings importiert.
                </p>
              )}
